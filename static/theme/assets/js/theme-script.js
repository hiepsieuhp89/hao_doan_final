...
function checkout_old(key){	
	...
	/*else if(text_email.length < 4 ||text_email.length >30 || validate_getkt.test(get_kt)){		
		...
		$('#dieukhoan').focus();
		$('#dieukhoan-error1').html('Để đặt hàng thành công bạn phải đồng ý với các điều khoản của Whaale.store');
		return false;
		}else{
		if(key==0){
			//form mua nhanh
...
function ShowResult(arrProducts) {
	...
	if (arrProducts.length != 0) {
		...
		$.each(arrProducts, function(i, index){
			...
			"<span class='pull-right'>Giá: <strong>"+addCommas(index.price)+"đ</strong></span></a>"+
			"</li>";
